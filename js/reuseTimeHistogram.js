...
function update(dataFilePath, binWidth){
    ...
    d3.csv(dataFilePath, function(data) {
        ...
        var color = d3.scaleSequential().domain([0, Math.max.apply(null, x.ticks(maxDayValue/binWidth))])
        ...
        // Compute kernel density estimation (4 is the bandwidth used for smoothing)
        smoothness = 4;     // bandwidth
...
.append("text")
    ...
    .attr("x", width/2)
    .attr("y", -height*0.2)
    .text("Histogram with density curve for cups reuse time")
        .style("text-anchor", "middle");
...
update('../data files/reuseDurationsFromSale.csv', 1);

// Listen to the button -> update if user change it
d3.select("#binWidth").on("change", function() {
    // checking which radio button is selected
    var isSaleChecked = document.getElementById('saleRadio').checked;
    var source = isSaleChecked ? '../data files/reuseDurationsFromSale.csv' : '../data files/reuseDurationsFromReturn.csv';
...
d3.selectAll(".source").on("change",function() {
    // checking which radio button is selected
    var source = this.value == 0 ? '../data files/reuseDurationsFromSale.csv' : '../data files/reuseDurationsFromReturn.csv';
